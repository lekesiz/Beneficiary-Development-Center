...
const shortAnswerSchema = z.object({
  correct_answers: z.array(z.string().min(1, 'Kabul edilebilir cevap boş olamaz')).min(1, 'En az 1 kabul edilebilir cevap gereklidir'),
...
const QuestionForm: React.FC<QuestionFormProps> = ({ mode }) => {
  ...
  const { evaluationId, questionId } = useParams<{ evaluationId: string; questionId?: string }>();
  ...
  const { fields: hintFields, append: addHint, remove: removeHint } = useFieldArray({
    control,
    name: 'hints'
  });
        ...
        <Card className="p-6">
          <div className="space-y-4">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <Badge variant={watch('difficulty_level') === 'easy' ? 'success' : watch('difficulty_level') === 'medium' ? 'warning' : 'danger'}>
