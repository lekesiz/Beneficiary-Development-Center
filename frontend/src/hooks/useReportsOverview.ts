...
const reportsOverviewApi = {
  ...
  getReportsOverview: async (filters?: ReportsOverviewFilters): Promise<ReportsOverviewResponse> => {
    ...
    const response = await apiClient.get(`${API_BASE_URL}/reports/overview?${params}`);
    return response.data;
  },

  // Add coach note
  addCoachNote: async (studentId: number, note: string): Promise<{ success: boolean; note: CoachNote }> => {
    const response = await apiClient.post(`${API_BASE_URL}/reports/students/${studentId}/note`, {
      note
    });
    return response.data;
  },
  ...
  exportBatchReports: async ({ student_ids, format = 'json' }: BatchExportRequest) => {
    ...
    link.href = url;
    link.setAttribute('download', `student_reports_batch_${new Date().toISOString().split('T')[0]}.${format}`);
...
export const useAddCoachNote = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['development-report', data.note.student_id] });
    }
  });
};
