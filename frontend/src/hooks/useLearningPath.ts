...
const learningPathApi = {
  ...
  createLearningPath: async (evaluationId: number, attemptId: number): Promise<LearningPath> => {
    const response = await apiClient.post(
      `${API_BASE_URL}/evaluations/${evaluationId}/attempts/${attemptId}/learning-path`
    );
  ...
  getLearningPath: async (pathId: number): Promise<LearningPath> => {
    const response = await apiClient.get(`${API_BASE_URL}/learning-paths/${pathId}`);
    return response.data;
  },

  // Get user's learning paths
  getMyLearningPaths: async (filters?: { status?: string; evaluation_id?: number }): Promise<LearningPath[]> => {
    const params = new URLSearchParams();
    if (filters?.status) params.append('status', filters.status);
    if (filters?.evaluation_id) params.append('evaluation_id', filters.evaluation_id.toString());
    
    const response = await apiClient.get(`${API_BASE_URL}/learning-paths/my-paths?${params}`);
    return response.data;
  },

  // Accept a learning path
  acceptLearningPath: async (pathId: number, customizationNotes?: string): Promise<LearningPath> => {
    const response = await apiClient.post(`${API_BASE_URL}/learning-paths/${pathId}/accept`, {
  ...
  updateLearningPath: async (pathId: number, updates: LearningPathUpdateParams): Promise<LearningPath> => {
    const response = await apiClient.put(`${API_BASE_URL}/learning-paths/${pathId}`, updates);
    return response.data;
  },
  ...
  updateMilestoneProgress: async (
    ...
    const response = await apiClient.put(
      `${API_BASE_URL}/learning-paths/${pathId}/milestones/${milestoneId}/progress`,
      params
    );
    return response.data;
  },
  ...
  provideFeedback: async (
    ...
    const response = await apiClient.post(`${API_BASE_URL}/learning-paths/${pathId}/feedback`, {
      feedback,
      rating
    });
    return response.data;
  ...
  getStatistics: async () => {
    const response = await apiClient.get(`${API_BASE_URL}/learning-paths/statistics`);
    return response.data;
  }
};
...
export const useCreateLearningPath = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(['learning-path', data.id], data);
    }
  });
};
...
export const useMyLearningPaths = (filters?: { status?: string; evaluation_id?: number }) => {
  return useQuery({
    queryKey: ['learning-paths', 'my', filters],
...
export const useUpdateMilestoneProgress = () => {
  ...
  return useMutation({
    mutationFn: ({ 
      ...
      learningPathApi.updateMilestoneProgress(pathId, milestoneId, { progress, notes }),
