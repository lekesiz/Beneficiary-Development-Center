...
describe('FileUpload', () => {
  ...
  it('handles file selection through input', async () => {
    ...
    const input = screen.getByRole('button', { name: /select files/i }).querySelector('input[type="file"]');
  ...
  it('opens file URL when view button is clicked', async () => {
    ...
    const uploadedFiles = [
      { id: '1', name: 'test.txt', size: 1000, type: 'text/plain', url: 'https://example.com/file.txt' },
    ];
