    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-3 border rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">Toplam Odak: {data.total_focus}</p>
          <p className="text-sm text-green-600">Tamamlanan: {data.completed}</p>
          <p className="text-sm text-yellow-600">Devam Eden: {data.in_progress}</p>
          <p className="text-sm text-blue-600">Ortalama İlerleme: {data.average_progress}%</p>
          <p className="text-sm font-medium mt-1">
            Ustalık: <span style={{ color: getMasteryColor(data.mastery_level) }}>
              {data.mastery_level}
            </span>
          </p>
        </div>
