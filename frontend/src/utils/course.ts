...
export const getCourseStatusInfo = (status: CourseStatus) => {
  return COURSE_STATUS_OPTIONS.find(option => option.value === status) || {
    value: status,
    label: status,
    color: 'gray'
  };
...
export const getCourseFormatInfo = (format: string) => {
  return COURSE_FORMAT_OPTIONS.find(option => option.value === format) || {
    value: format,
    label: format,
    icon: '📋'
  };
...
export const getDifficultyLevelInfo = (level: DifficultyLevel) => {
  return DIFFICULTY_LEVEL_OPTIONS.find(option => option.value === level) || {
    value: level,
    label: level,
    color: 'gray',
    order: 0
...
export const getAssessmentTypeInfo = (type: string) => {
  return ASSESSMENT_TYPE_OPTIONS.find(option => option.value === type) || {
    value: type,
    label: type,
    icon: '📋'
  };
...
export const formatPrerequisites = (prerequisites: string[]) => {
  ...
  return prerequisites.slice(0, -1).join(', ') + ' ve ' + prerequisites[prerequisites.length - 1];
};
 ...
 */
export const sortCourses = (courses: Course[], sortBy: string, sortOrder: 'asc' | 'desc' = 'asc') => {
  const sorted = [...courses].sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'difficulty_level':
        comparison = getCourseDifficultyScore(a.difficulty_level) - getCourseDifficultyScore(b.difficulty_level);
        break;
      case 'duration_hours':
