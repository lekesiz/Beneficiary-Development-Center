...
from .validation import BaseSchema, PaginationSchema, validate_no_sql_injection, validate_no_xss
...
class CourseBaseSchema(BaseSchema):
    ...
    subtitle = fields.String(validate=[validate.Length(max=300), validate_no_xss], allow_none=True)
    ...
    description = fields.String(validate=[validate.Length(max=5000), validate_no_xss], allow_none=True)
    ...
    outline = fields.List(
        fields.Dict(keys=fields.String(), values=fields.Raw()), validate=validate.Length(max=50), missing=[]
    )
    ...
    format = fields.String(
        validate=validate.OneOf(
            ["lecture", "workshop", "practical", "online", "self_paced", "hybrid", "seminar", "lab"]
        ),
        missing="lecture",
        error_messages={"invalid": "Invalid course format"},
    )

    difficulty_level = fields.String(
        validate=validate.OneOf(["beginner", "intermediate", "advanced", "expert"]),
        missing="beginner",
        error_messages={"invalid": "Invalid difficulty level"},
    )

    status = fields.String(validate=validate.OneOf(["draft", "published", "archived"]), missing="draft")

    duration_hours = fields.Float(
        validate=validate.Range(min=0.5, max=200),
        missing=1.0,
        error_messages={"invalid": "Duration must be between 0.5 and 200 hours"},
    )

    duration_weeks = fields.Integer(
        validate=validate.Range(min=1, max=52),
        missing=1,
        error_messages={"invalid": "Duration must be between 1 and 52 weeks"},
    )

    order_index = fields.Integer(validate=validate.Range(min=0), missing=0)

    # Content and resources
    content_url = fields.URL(validate=validate.Length(max=500), allow_none=True)

    video_url = fields.URL(validate=validate.Length(max=500), allow_none=True)
    ...
    assessment_type = fields.String(
        validate=validate.OneOf(["quiz", "project", "presentation", "exam", "assignment"]), allow_none=True
    )
    ...
    # Capacity
    min_participants = fields.Integer(validate=validate.Range(min=1, max=1000), missing=1)

    max_participants = fields.Integer(validate=validate.Range(min=1, max=1000), allow_none=True)

    # Additional information
    instructor_id = fields.Integer(validate=validate.Range(min=1), allow_none=True)

    tags = fields.List(fields.String(validate=validate.Length(max=50)), validate=validate.Length(max=20), missing=[])

    thumbnail_url = fields.URL(validate=validate.Length(max=500), allow_none=True)

    course_metadata = fields.Dict(missing={}, error_messages={"invalid": "Metadata must be a valid JSON object"})
...
class CourseCreateSchema(CourseBaseSchema):
    ...
    program_id = fields.Integer(
        ...
        error_messages={"required": "Program ID is required", "invalid": "Invalid program ID"},
    )
...
class CourseUpdateSchema(BaseSchema):
    ...
    subtitle = fields.String(
        validate=[validate.Length(max=300), validate_no_xss], allow_none=True, missing=fields.missing_
    )

    description = fields.String(
        validate=[validate.Length(max=5000), validate_no_xss], allow_none=True, missing=fields.missing_
    )
    ...
    difficulty_level = fields.String(
        validate=validate.OneOf(["beginner", "intermediate", "advanced", "expert"]), missing=fields.missing_
    )

    status = fields.String(validate=validate.OneOf(["draft", "published", "archived"]), missing=fields.missing_)

    duration_hours = fields.Float(validate=validate.Range(min=0.5, max=200), missing=fields.missing_)

    duration_weeks = fields.Integer(validate=validate.Range(min=1, max=52), missing=fields.missing_)

    tags = fields.List(
        fields.String(validate=validate.Length(max=50)), validate=validate.Length(max=20), missing=fields.missing_
    )
    ...
    passing_score = fields.Float(validate=validate.Range(min=0, max=100), missing=fields.missing_)

    # Capacity
    min_participants = fields.Integer(validate=validate.Range(min=1, max=1000), missing=fields.missing_)
    max_participants = fields.Integer(
        validate=validate.Range(min=1, max=1000), allow_none=True, missing=fields.missing_
    )

    # Additional fields
    instructor_id = fields.Integer(validate=validate.Range(min=1), allow_none=True, missing=fields.missing_)
...
class CourseQuerySchema(PaginationSchema):
    ...
    instructor_id = fields.Integer(validate=validate.Range(min=1), missing=fields.missing_)
    ...
    min_duration_hours = fields.Float(validate=validate.Range(min=0), missing=fields.missing_)

    max_duration_hours = fields.Float(validate=validate.Range(min=0), missing=fields.missing_)
...
class CourseSessionSchema(BaseSchema):
    ...
    description = fields.String(validate=[validate.Length(max=1000), validate_no_xss], allow_none=True)

    session_date = fields.Date(
        required=True,
        format="%Y-%m-%d",
        error_messages={"required": "Session date is required", "invalid": "Invalid date format. Use YYYY-MM-DD"},
    )

    start_time = fields.Time(
        required=True,
        format="%H:%M",
        error_messages={"required": "Start time is required", "invalid": "Invalid time format. Use HH:MM"},
    )

    end_time = fields.Time(
        required=True,
        format="%H:%M",
        error_messages={"required": "End time is required", "invalid": "Invalid time format. Use HH:MM"},
    )

    duration_hours = fields.Float(
        validate=validate.Range(min=0.25, max=12),
        error_messages={"invalid": "Session duration must be between 0.25 and 12 hours"},
    )

    location = fields.String(validate=[validate.Length(max=200), validate_no_xss], allow_none=True)

    online_link = fields.URL(validate=validate.Length(max=500), allow_none=True)
    ...
    materials = fields.List(fields.Dict(), validate=validate.Length(max=20), missing=[])
...
class CourseReorderSchema(BaseSchema):
    ...
    course_orders = fields.Dict(
        ...
        required=True,
        error_messages={"required": "Course orders mapping is required"},
    )
...
class CourseDuplicateSchema(BaseSchema):
    ...
    title = fields.String(validate=[validate.Length(min=3, max=200), validate_no_xss], missing=None)

    program_id = fields.Integer(validate=validate.Range(min=1), missing=None)
...
class CourseResponseSchema(BaseSchema):
    ...
    def format_response(self, data, **kwargs):
        ...
        string_fields = ["subtitle", "description", "content_url", "video_url", "thumbnail_url", "instructor_name"]
