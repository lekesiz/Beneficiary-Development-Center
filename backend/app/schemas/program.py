...
class ProgramCreateSchema(Schema):
    ...
    category = fields.Str(
        required=False, validate=validate.OneOf(["education", "vocational", "health", "social", "economic", "other"])
    )
    status = fields.Str(required=False, validate=validate.OneOf(["draft", "active", "completed", "suspended"]))
...
class ProgramQuerySchema(Schema):
    ...
    sort_by = fields.Str(required=False, validate=validate.OneOf(["name", "created_at", "start_date", "status"]))
