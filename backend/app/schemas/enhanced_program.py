...
class ProgramBaseSchema(BaseSchema):
    ...
    code = fields.String(
        ...
        error_messages={"invalid": "Program code must be 3-50 characters long"},
    )

    description = fields.String(
        validate=[validate.Length(max=5000), validate_no_xss],
        allow_none=True,
        error_messages={"invalid": "Description must not exceed 5000 characters"},
    )
    ...
    objectives = fields.List(
        ...
        missing=[],
        error_messages={"invalid": "Maximum 20 objectives allowed, each up to 500 characters"},
    )

    program_type = fields.String(
        validate=validate.OneOf(
            ["education", "training", "workshop", "bootcamp", "certification", "mentorship", "internship"]
        ),
        error_messages={"invalid": "Invalid program type"},
    )

    status = fields.String(
        validate=validate.OneOf(["draft", "published", "active", "completed", "archived"]),
        missing="draft",
        error_messages={"invalid": "Invalid program status"},
    )
    ...
    online_link = fields.URL(
        ...
        allow_none=True,
        error_messages={"invalid": "Invalid URL format for online link"},
    )

    min_participants = fields.Integer(
        validate=validate.Range(min=1, max=1000),
        missing=1,
        error_messages={"invalid": "Minimum participants must be between 1 and 1000"},
    )

    max_participants = fields.Integer(
        validate=validate.Range(min=1, max=10000),
        missing=50,
        error_messages={"invalid": "Maximum participants must be between 1 and 10000"},
    )
    ...
    price = fields.Decimal(
        ...
        missing=0,
        error_messages={"invalid": "Price must be a valid amount up to 999,999.99"},
    )

    currency = fields.String(
        validate=[validate.Length(equal=3), validate.Regexp(r"^[A-Z]{3}$")],
        missing="USD",
        error_messages={"invalid": "Currency must be a 3-letter ISO code (e.g., USD, EUR)"},
    )
    ...
    tags = fields.List(
        ...
        validate=validate.Length(max=20),
        missing=[],
        error_messages={"invalid": "Maximum 20 tags allowed, each up to 50 characters"},
    )

    cover_image_url = fields.URL(validate=validate.Length(max=500), allow_none=True)

    coordinator_id = fields.Integer(validate=validate.Range(min=1), allow_none=True)

    metadata = fields.Dict(missing={}, error_messages={"invalid": "Metadata must be a valid JSON object"})
...
class ProgramCreateSchema(ProgramBaseSchema):
    ...
    start_date = fields.Date(
        required=True,
        format="%Y-%m-%d",
        error_messages={"required": "Start date is required", "invalid": "Invalid date format. Use YYYY-MM-DD"},
    )

    end_date = fields.Date(
        required=True,
        format="%Y-%m-%d",
        error_messages={"required": "End date is required", "invalid": "Invalid date format. Use YYYY-MM-DD"},
    )

    enrollment_start = fields.Date(
        format="%Y-%m-%d", allow_none=True, error_messages={"invalid": "Invalid date format. Use YYYY-MM-DD"}
    )
    ...
    requirements = fields.Dict(
        missing={"age_min": None, "age_max": None, "education_level": None, "skills": [], "other": []}
    )
...
class ProgramUpdateSchema(ProgramBaseSchema):
    ...
    start_date = fields.Date(format="%Y-%m-%d", missing=fields.missing_)

    end_date = fields.Date(format="%Y-%m-%d", missing=fields.missing_)

    enrollment_start = fields.Date(format="%Y-%m-%d", allow_none=True, missing=fields.missing_)

    enrollment_end = fields.Date(format="%Y-%m-%d", allow_none=True, missing=fields.missing_)
...
class ProgramQuerySchema(PaginationSchema):
    ...
    status = fields.String(
        validate=validate.OneOf(["draft", "published", "active", "completed", "archived"]), missing=fields.missing_
    )
    ...
    min_price = fields.Decimal(places=2, validate=validate.Range(min=0), missing=fields.missing_)

    max_price = fields.Decimal(places=2, validate=validate.Range(min=0), missing=fields.missing_)

    coordinator_id = fields.Integer(validate=validate.Range(min=1), missing=fields.missing_)

    tags = fields.List(fields.String(), missing=fields.missing_)

    start_date_from = fields.Date(format="%Y-%m-%d", missing=fields.missing_)

    start_date_to = fields.Date(format="%Y-%m-%d", missing=fields.missing_)
...
class ProgramBatchOperationSchema(BaseSchema):
    ...
    confirm = fields.Boolean(
        required=True, error_messages={"required": "Confirmation is required for batch operations"}
    )

    reason = fields.String(validate=[validate.Length(max=500), validate_no_xss], missing=None)
