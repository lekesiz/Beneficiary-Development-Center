...
class EvaluationBaseSchema(BaseSchema):
    ...
    evaluation_type = fields.String(
        validate=validate.OneOf([
            'quiz', 'exam', 'assessment', 'survey', 
            'diagnostic', 'placement', 'certification'
        ]),
        missing='quiz',
        error_messages={
            "invalid": "Invalid evaluation type"
        }
    )
    ...
    time_limit_minutes = fields.Integer(
        ...
        allow_none=True,
        error_messages={
            "invalid": "Time limit must be between 0 and 480 minutes"
        }
    )
    ...
    max_attempts = fields.Integer(
        ...
        missing=3,
        error_messages={
            "invalid": "Maximum attempts must be between 1 and 10"
        }
    )
    ...
    attempt_cooldown_hours = fields.Integer(
        ...
        missing=0,
        error_messages={
            "invalid": "Cooldown period must be between 0 and 168 hours"
        }
    )
    ...
    scoring_method = fields.String(
        validate=validate.OneOf(['highest', 'latest', 'average']),
        missing='highest'
    )
    ...
    difficulty_adjustment_factor = fields.Float(
        validate=validate.Range(min=0.1, max=2.0),
        missing=1.0
    )
...
class QuestionBaseSchema(BaseSchema):
    ...
    question_type = fields.String(
        ...
        validate=validate.OneOf([
            ...
            'essay', 'matching', 'ordering', 'fill_blank'
        ]),
        error_messages={
    ...
    difficulty_level = fields.String(
        validate=validate.OneOf(['easy', 'medium', 'hard', 'expert']),
        missing='medium'
    )
    ...
    points = fields.Integer(
        ...
        missing=1,
        error_messages={
            "invalid": "Points must be between 1 and 100"
        }
    )
    ...
    explanation = fields.String(
        validate=[validate.Length(max=2000), validate_no_xss],
        allow_none=True
    )
    ...
    time_limit_seconds = fields.Integer(
        validate=validate.Range(min=0, max=3600),  # Max 1 hour per question
        allow_none=True
    )
    
    # Categories and tags
    category = fields.String(
        validate=[validate.Length(max=100), validate_no_xss],
        allow_none=True
    )
...
class AttemptStartSchema(BaseSchema):
    ...
    acknowledgment = fields.Boolean(
        required=True,
        error_messages={
            "required": "You must acknowledge the evaluation rules"
        }
    )
...
class ResponseSubmitSchema(BaseSchema):
    ...
    response_data = fields.Dict(
        required=True,
        error_messages={
            "required": "Response data is required"
        }
    )
...
class AttemptSubmitSchema(BaseSchema):
    ...
    responses = fields.List(
        ...
        error_messages={
            "validator_failed": "At least one response is required"
        }
    )
    ...
    submit_confirmation = fields.Boolean(
        required=True,
        error_messages={
            "required": "Submission confirmation is required"
        }
    )
