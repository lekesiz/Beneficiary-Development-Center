...
class LearningPath(TenantBaseModel, TimestampMixin):
    ...
    evaluation_attempt = relationship("EvaluationAttempt")
...
class LearningPathUpdate(TenantBaseModel, TimestampMixin):
    ...
    # Update Type
    update_type = Column(String(50), nullable=False)  # 'add_milestone', 'reorder', 'obsolete_milestone', 'modify_milestone'
...
class MilestoneProgress(TenantBaseModel, TimestampMixin):
    ...
    # Relationships
    milestone = relationship("LearningMilestone", backref="progress_records")
    user = relationship("User", backref="milestone_progress")
