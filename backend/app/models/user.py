...
class User(TenantBaseModel):
    ...
    last_login_ip = Column(String(45))  # IPv6 max length is 45 chars
    ...
    created_beneficiaries = relationship("Beneficiary", foreign_keys="Beneficiary.created_by", back_populates="creator")
    assigned_beneficiaries = relationship(
        "Beneficiary", foreign_keys="Beneficiary.assigned_trainer_id", back_populates="assigned_trainer"
    )
    # appointments_as_trainer = relationship('Appointment', foreign_keys='Appointment.trainer_id')
    created_programs = relationship("Program", foreign_keys="Program.created_by", back_populates="creator")
    coordinated_programs = relationship("Program", foreign_keys="Program.coordinator_id", back_populates="coordinator")
    instructed_courses = relationship("Course", foreign_keys="Course.instructor_id", back_populates="instructor")
    created_courses = relationship("Course", foreign_keys="Course.created_by", back_populates="creator")
    suggested_learning_path_updates = relationship(
        "LearningPathUpdate", foreign_keys="LearningPathUpdate.suggested_by_user_id", back_populates="suggested_by"
    )
    ...
    evaluation_attempts = relationship("EvaluationAttempt", back_populates="user")
    ...
    # sent_messages = relationship defined via backref in ChatMessage
    # received_messages = relationship defined via backref in ChatMessage

    # Add unique constraint for email within tenant
    __table_args__ = (db.UniqueConstraint("email", "tenant_id", name="_email_tenant_uc"),)
