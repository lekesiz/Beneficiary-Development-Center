...
from app.models.learning_path import LearningPath, LearningMilestone, LearningPathStatus, LearningPathUpdate
...
from app.exceptions import NotFoundError, BadRequestError, ForbiddenError
...
class LearningPathService(BaseService):
    ...
    def create_learning_path(
        ...
        insights = adaptive_evaluation_service.get_learning_insights(
            tenant_id, evaluation_id, attempt_id
        )
        ...
        existing_path = self.db.query(LearningPath).filter(
            LearningPath.evaluation_attempt_id == attempt_id,
    ...
    def update_learning_path(
        ...
        # Allowed updates
        allowed_fields = [
            'title', 'description', 'objective', 'estimated_hours_per_week',
            'customization_notes', 'weekly_schedule'
        ]
    ...
    def update_milestone_progress(
        ...
        milestone = self.db.query(LearningMilestone).filter(
    ...
    def _determine_experience_level(self, user_id: int) -> str:
        ...
        completed_count = self.db.query(EvaluationAttempt).filter(
    ...
    def suggest_learning_path_updates(
        ...
        profile = student_profile_service.get_student_profile(
            student_id, tenant_id, requesting_user
        )
    ...
    def _generate_path_suggestions(
        ...
        if strategy == 'crisis_intervention':
            suggestions.append(self._create_crisis_intervention_suggestion(
                learning_path, profile
            ))
        
        # 2. Performance Recovery Strategy
        elif strategy == 'performance_recovery':
            suggestions.append(self._create_performance_recovery_suggestion(
                learning_path, profile
            ))
        
        # 3. Accelerated Learning Strategy
        elif strategy == 'accelerated_learning':
            suggestions.append(self._create_advanced_acceleration_suggestion(
                learning_path, profile
            ))
        
        # 4. Balanced Development Strategy
        elif strategy == 'balanced_development':
            suggestions.append(self._create_balanced_development_suggestion(
                learning_path, profile
            ))
        ...
        # 5. Standard interventions for specific issues
            ...
            if scores.get('risk_score') == 'High':
                suggestions.append(self._create_support_milestone_suggestion(
                    learning_path, ai_analysis, scores
                ))
            
            # Weakness reinforcement
            weaknesses = ai_analysis.get('weaknesses', [])
            if weaknesses:
                suggestions.append(self._create_reinforcement_suggestion(
                    learning_path, weaknesses, ai_analysis
                ))
            ...
            if critical_interventions:
                suggestions.append(self._create_intervention_based_suggestion(
                    learning_path, critical_interventions
                ))
    ...
    def _determine_intervention_strategy(self, profile: Dict[str, Any]) -> str:
        ...
        if (scores.get('risk_score') == 'High' and 
            scores.get('engagement_score', 0) < 30 and 
            days_inactive > 21):
        ...
        if (scores.get('performance_index', 0) >= 95 and 
            scores.get('completion_rate', 0) >= 90):
    ...
    def _create_support_milestone_suggestion(
        ...
        existing = self.db.query(LearningPathUpdate).filter(
        ...
        if scores.get('engagement_score', 0) < 50:
            support_milestones.append({
                ...
                'resources': [
                    {'type': 'video', 'title': 'Etkili Hedef Belirleme Teknikleri', 'url': '#'}
                ]
            })
                ...
                for milestone in learning_path.milestones:
                    if milestone.status != 'completed':
                        milestone_updates[milestone.id] = {
                            ...
                            'activities': milestone.activities[:2] if len(milestone.activities) > 2 else milestone.activities
                        }
    ...
    def get_pending_updates(
        ...
        # Get pending updates
        updates = self.db.query(LearningPathUpdate).filter(
    ...
    def apply_learning_path_update(
        self,
        update_id: int,
        tenant_id: int,
        user: User
        ...
        # Get update
        update = self.db.query(LearningPathUpdate).filter(
        ...
        # Get learning path
        learning_path = self.db.query(LearningPath).filter(
            LearningPath.id == update.learning_path_id
        ).first()
    ...
    def _apply_modify_milestone_update(self, learning_path: LearningPath, update: LearningPathUpdate):
        ...
        for milestone_id, changes in update.milestone_updates.items():
            milestone = self.db.query(LearningMilestone).filter(
                LearningMilestone.id == int(milestone_id),
                LearningMilestone.learning_path_id == learning_path.id
            ).first()
    ...
    def _create_performance_recovery_suggestion(
        ...
        return LearningPathUpdate(
            ...
            ai_analysis_data={
                ...
                'average_drop': sum(recent_scores[i] - recent_scores[i+1] for i in range(len(recent_scores)-1)) / (len(recent_scores)-1) if len(recent_scores) > 1 else 0
            }
        )
