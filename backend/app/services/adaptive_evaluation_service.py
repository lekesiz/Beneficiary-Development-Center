...
from app.models.evaluation import Evaluation, Question, EvaluationAttempt, QuestionResponse, QuestionBank
...
class AdaptiveEvaluationService(BaseService):
    ...
    def get_next_question(
        self, tenant_id: int, evaluation_id: int, attempt_id: int, user_id: int
    ) -> Optional[Question]:
        """
        ...
        evaluation = (
            self.db.query(Evaluation).filter(Evaluation.tenant_id == tenant_id, Evaluation.id == evaluation_id).first()
        )
        ...
        answered_question_ids = (
            self.db.query(QuestionResponse.question_id)
            .filter(QuestionResponse.evaluation_attempt_id == attempt_id)
            .subquery()
        )
        ...
        next_difficulty = self._determine_next_difficulty(recent_responses, current_difficulty, evaluation)

        # Select next question based on difficulty
        next_question = self._select_next_question(evaluation_id, answered_question_ids, next_difficulty, evaluation)

        # Log the selection for analytics
        if next_question:
            self._log_question_selection(attempt_id, next_question.id, next_difficulty, recent_responses)

        return next_question

    def _get_recent_responses(self, attempt_id: int, limit: int = 5) -> List[Dict[str, Any]]:
        """Get recent question responses for analysis."""
        responses = (
            self.db.query(QuestionResponse, Question)
    ...
    def _determine_next_difficulty(
        self, recent_responses: List[Dict[str, Any]], current_difficulty: str, evaluation: Evaluation
    ) -> str:
        """Determine next difficulty using AI service."""
        evaluation_context = {
            "subject": evaluation.title,
            "description": evaluation.description,
            "total_questions": self.db.query(Question).filter(Question.evaluation_id == evaluation.id).count(),
        }

        # Use AI service to determine next difficulty
        return ai_service.determine_next_difficulty(recent_responses, current_difficulty, evaluation_context)

    def _select_next_question(
        self, evaluation_id: int, answered_question_ids, difficulty: str, evaluation: Evaluation
    ) -> Optional[Question]:
        """Select the next question based on difficulty and other criteria."""
        # Base query for unanswered questions
        query = self.db.query(Question).filter(
            Question.evaluation_id == evaluation_id, ~Question.id.in_(answered_question_ids)
        )

        # First try to get a question with the target difficulty
        question = query.filter(Question.difficulty_level == difficulty).order_by(func.random()).first()
        ...
        if not question:
            ...
            if difficulty == "medium":
                # Try easy or hard
                question = query.filter(Question.difficulty_level.in_(["easy", "hard"])).order_by(func.random()).first()
            elif difficulty == "easy":
                # Try medium
                question = query.filter(Question.difficulty_level == "medium").order_by(func.random()).first()
    ...
    def _log_question_selection(
        self, attempt_id: int, question_id: int, selected_difficulty: str, recent_responses: List[Dict[str, Any]]
    ):
    ...
    def get_learning_insights(self, tenant_id: int, evaluation_id: int, attempt_id: int) -> Dict[str, Any]:
        ...
        responses = (
            self.db.query(QuestionResponse, Question)
            .join(Question, QuestionResponse.question_id == Question.id)
            .filter(QuestionResponse.evaluation_attempt_id == attempt_id, Question.evaluation_id == evaluation_id)
        ...
        for response, question in responses:
            ...
            topic = question.question_data.get("topic") or question.question_metadata.get("subject") or "Genel"
...
# Service instance - will be created when needed with proper context
