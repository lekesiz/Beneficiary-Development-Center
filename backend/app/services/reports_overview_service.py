...
class ReportsOverviewService(BaseService):
    ...
    def get_students_overview(self, tenant_id: int, requesting_user: User, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if sort_by == "risk_score":
            ...
            report_summaries.sort(key=lambda x: risk_order.get(x["risk_score"].lower(), 0), reverse=sort_desc)
        elif sort_by == "performance":
            report_summaries.sort(key=lambda x: x["performance_index"], reverse=sort_desc)
        elif sort_by == "name":
            report_summaries.sort(key=lambda x: x["student_name"], reverse=sort_desc)
    ...
    def _generate_student_summary(self, student: User, tenant_id: int) -> Dict[str, Any]:
        ...
        recent_attempts = (
            ...
            .order_by(EvaluationAttempt.completed_at.desc())
            .limit(5)
            .all()
        )
    ...
    def _get_student_enrollments(self, student_id: int, tenant_id: int) -> List[Dict[str, Any]]:
    ...
    def add_coach_note(self, student_id: int, tenant_id: int, requesting_user: User, note: str) -> Dict[str, Any]:
        """
    ...
    def export_reports_batch(
        self, student_ids: List[int], tenant_id: int, requesting_user: User, format: str = "json"
    ) -> Dict[str, Any]:
        """
        ...
        for student_id in student_ids:
            try:
                report = report_service.generate_development_report(
                    user_id=student_id, tenant_id=tenant_id, requesting_user=requesting_user, date_range_days=30
                )
                reports.append(report)
            except Exception as e:
                self.logger.error(f"Failed to generate report for student {student_id}: {str(e)}")
                reports.append({"student_id": student_id, "error": str(e)})
...
# Initialize service
# reports_overview_service = ReportsOverviewService()  # Should be instantiated in request context
