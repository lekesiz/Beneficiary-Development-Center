...
class AIService:
    ...
    def determine_next_difficulty(
        ...
        prompt = self._build_difficulty_prompt(
            recent_responses, 
            accuracy, 
            current_difficulty,
            evaluation_context
    ...
    def _generate_fallback_insights(self, performance_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'visual_indicators': {
                'trend': 'stable',
                'level': self._determine_mastery_level(summary['accuracy'])
            }
        }
    ...
    def _generate_recommendations(
        ...
        if weak_topics:
            recommendations.append(
                f"Focus on improving in: {', '.join(weak_topics[:3])}"
            )
        
        if strong_topics:
            recommendations.append(
                f"Great performance in: {', '.join(strong_topics[:3])}"
            )
    ...
    def _generate_fallback_learning_path(
        ...
        milestone_templates = [
            {
                'title': 'Temel Kavramları Gözden Geçirme',
                'skill_focus': 'Temel bilgi',
                'week': 1
            },
            {
                'title': 'Pratik Uygulamalar',
                'skill_focus': 'Uygulama becerisi',
                'week': 2
            },
            {
                'title': 'İleri Seviye Konular',
                'skill_focus': 'Derinlemesine anlayış',
                'week': 3
            },
            {
                'title': 'Proje ve Değerlendirme',
                'skill_focus': 'Sentez ve değerlendirme',
                'week': 4
            }
        ]
        ...
        for week in range(1, 5):
            weekly_schedule[f'week_{week}'] = {
                'monday': {
                    'activities': ['Teori çalışması'],
                    'duration': f'{hours_per_week / 3:.1f} saat'
                },
                'wednesday': {
                    'activities': ['Pratik uygulama'],
                    'duration': f'{hours_per_week / 3:.1f} saat'
                },
                'friday': {
                    'activities': ['Tekrar ve değerlendirme'],
                    'duration': f'{hours_per_week / 3:.1f} saat'
                }
            }
    ...
    def _generate_fallback_report(self, report_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report['recommendations']['support_needed'] = [
            'Haftalık ilerleme takibi',
            'Motivasyon desteği'
        ]
        
        # Add next evaluation focus
        report['next_evaluation_focus'] = [
            'Zayıf performans gösterilen konular',
    ...
    def _generate_fallback_profile_analysis(self, profile_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if activity_gap_days >= 14:
            ...
            alerts.append({
                'type': 'warning',
                'message': f'Son {activity_gap_days} gündür aktivite azalmış',
                'priority': 'medium'
            })
        ...
        if len(evaluations) >= 3:
            ...
            if all(recent_scores[i] < recent_scores[i+1] for i in range(len(recent_scores)-1)):
                alerts.append({
                    'type': 'danger',
                    'message': 'Son 3 değerlendirmede sürekli düşüş var',
                    'priority': 'high'
                })
        ...
        if scores.get('risk_score') == 'High':
            alerts.append({
                'type': 'danger',
                'message': 'Yüksek risk grubunda - Acil müdahale gerekiyor',
                'priority': 'high'
            })
        ...
        recommendations.extend([
            'Haftalık ilerleme değerlendirmeleri yapılmalı',
            'Akran öğrenmesi teşvik edilmeli'
        ])
