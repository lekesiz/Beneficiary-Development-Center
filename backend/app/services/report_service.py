...
class ReportService(BaseService):
    ...
    def generate_development_report(
        self, user_id: int, tenant_id: int, requesting_user: User, date_range_days: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        report_data = {
            "user_info": {
                ...
                "email": user.email,
                "learning_style": (
                    getattr(user.profile, "learning_style", "mixed") if hasattr(user, "profile") else "mixed"
                ),
            },
            "evaluation_results": evaluation_data,
            "learning_paths": learning_path_data,
            "performance_metrics": performance_metrics,
            "date_range": {"start": start_date.isoformat(), "end": end_date.isoformat(), "days": date_range_days},
        }
    ...
    def _get_evaluation_data(
        self, user_id: int, tenant_id: int, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        ...
        if attempts:
            ...
            for attempt in attempts:
                ...
                if eval_id not in evaluation_data["attempts_by_evaluation"]:
                    evaluation_data["attempts_by_evaluation"][eval_id] = {
                        "evaluation_title": (
                            attempt.evaluation.title if hasattr(attempt, "evaluation") else f"Evaluation {eval_id}"
                        ),
                        "attempts": [],
                    }
            ...
            adaptive_service = AdaptiveEvaluationService()
            for attempt in recent_attempts:
                insights = adaptive_service.get_learning_insights(tenant_id, attempt.evaluation_id, attempt.id)
                evaluation_data["insights"].append(
                    {"evaluation_id": attempt.evaluation_id, "attempt_id": attempt.id, "insights": insights}
                )
            ...
            for i, attempt in enumerate(sorted_attempts):
                evaluation_data["performance_trend"].append(
                    {"date": attempt.completed_at.isoformat(), "score": attempt.percentage_score, "index": i + 1}
                )
            ...
            for path in learning_paths:
                ...
                if path.feedback:
                    path_data["feedback_notes"].append(
                        {"path_id": path.id, "feedback": path.feedback, "rating": path.rating}
                    )
                if path.customization_notes:
                    path_data["feedback_notes"].append(
                        {"path_id": path.id, "notes": path.customization_notes, "type": "customization"}
                    )
    ...
    def _calculate_performance_metrics(
        self, evaluation_data: Dict[str, Any], learning_path_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def get_batch_reports(
        self, user_ids: List[int], tenant_id: int, requesting_user: User, date_range_days: int = 30
    ) -> List[Dict[str, Any]]:
        """
        ...
        for user_id in user_ids:
            try:
                report = self.generate_development_report(user_id, tenant_id, requesting_user, date_range_days)
                reports.append(report)
            except Exception as e:
                self.logger.error(f"Failed to generate report for user {user_id}: {str(e)}")
                reports.append({"user_id": user_id, "error": str(e), "status": "failed"})
