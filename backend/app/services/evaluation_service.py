...
from app.models.evaluation import (
    Evaluation, Question, EvaluationAttempt, QuestionResponse, QuestionBank,
    EvaluationStatus, QuestionType, AttemptStatus
)
...
class EvaluationService(BaseService):
    ...
    def get_all(self, tenant_id: int, filters: Dict[str, Any] = None, user: User = None) -> Dict[str, Any]:
        ...
        if filters.get('search'):
            ...
            query = query.filter(
                or_(
                    Evaluation.title.ilike(search_term),
                    Evaluation.description.ilike(search_term)
                )
            )
...
class EvaluationAttemptService(BaseService):
    ...
    def start_attempt(self, evaluation_id: int, tenant_id: int, user: User) -> EvaluationAttempt:
        ...
        existing_attempts = self.db.query(EvaluationAttempt).filter(
            and_(
        ...
        in_progress = self.db.query(EvaluationAttempt).filter(
            and_(
                EvaluationAttempt.evaluation_id == evaluation_id,
                EvaluationAttempt.user_id == user.id,
                EvaluationAttempt.tenant_id == tenant_id,
    ...
    def get_user_attempts(self, evaluation_id: int, user_id: int, tenant_id: int) -> List[EvaluationAttempt]:
        ...
        return self.db.query(EvaluationAttempt).filter(
            and_(
                EvaluationAttempt.evaluation_id == evaluation_id,
                EvaluationAttempt.user_id == user_id,
                EvaluationAttempt.tenant_id == tenant_id
            )
        ).order_by(desc(EvaluationAttempt.attempt_number)).all()
...
class QuestionResponseService(BaseService):
    ...
    def save_response(self, attempt_id: int, question_id: int, tenant_id: int, 
                     response_data: Dict[str, Any], user: User) -> QuestionResponse:
        ...
        question = self.db.query(Question).filter(
            and_(Question.id == question_id, Question.tenant_id == tenant_id)
        ).first()
