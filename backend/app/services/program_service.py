...
class ProgramService(BaseService[Program]):
    ...
    def get_all(
        ...
        Args:
            ...
            sort_by: Field to sort by (title, created_at, start_date, status, enrollment_count)
            sort_order: Sort order (asc or desc)

        Returns:
            List of programs
        """
        query = self.db.query(Program).filter(Program.tenant_id == tenant_id, Program.deleted_at.is_(None))
        ...
        if upcoming_only:
            ...
            query = query.filter(
                Program.status.in_([ProgramStatus.PUBLISHED, ProgramStatus.ACTIVE]), Program.start_date > today
            )

        if active_only:
            today = datetime.today().date()
            query = query.filter(
                Program.status == ProgramStatus.ACTIVE, Program.start_date <= today, Program.end_date >= today
            )
    ...
    def get_by_code(self, tenant_id: int, code: str, user: User) -> Program:
        ...
        program = (
            self.db.query(Program)
            .filter(Program.code == code, Program.tenant_id == tenant_id, Program.deleted_at.is_(None))
            .first()
        )
    ...
    def create(self, tenant_id: int, data: Dict[str, Any], user: User) -> Program:
        ...
        # Create program
        program_data = {"tenant_id": tenant_id, "created_by": user.id, **data}
        ...
        # Send notification to the creator and managers/admins
        try:
            notification_service_instance = NotificationService(db_session=self.db)
            notification_service_instance.notify_program_created(
                program_id=program.id, program_title=program.title, created_by_id=user.id, tenant_id=tenant_id
            )
        except Exception as e:
            logger.error(f"Failed to send program creation notification: {str(e)}")
            # Don't fail the operation if notification fails

        return program

    def update(self, tenant_id: int, program_id: int, data: Dict[str, Any], user: User) -> Program:
        """
        Update a program.
    ...
    def delete(self, tenant_id: int, program_id: int, user: User) -> bool:
        ...
        if active_enrollments:
            raise BadRequestError(f"Cannot delete program with {len(active_enrollments)} active enrollments")
    ...
    def update_status(self, tenant_id: int, program_id: int, new_status: ProgramStatus, user: User) -> Program:
        """
    ...
    def add_course(self, tenant_id: int, program_id: int, course_data: Dict[str, Any], user: User) -> Program:
        """
    ...
    def get_statistics(self, tenant_id: int, user: User) -> Dict[str, Any]:
        ...
        status_counts = (
            self.db.query(Program.status, func.count(Program.id))
            .filter(Program.tenant_id == tenant_id, Program.deleted_at.is_(None))
        ...
        type_counts = (
            self.db.query(Program.program_type, func.count(Program.id))
            .filter(Program.tenant_id == tenant_id, Program.deleted_at.is_(None))
        ...
        today = datetime.today().date()
        upcoming_count = (
            self.db.query(func.count(Program.id))
            .filter(
