...
class StudentProfileService(BaseService):
    ...
    def _get_student_info(self, student: User) -> Dict[str, Any]:
        ...
        return {
            ...
            'learning_style': getattr(student.profile, 'learning_style', 'mixed') if hasattr(student, 'profile') else 'mixed'
        }
    ...
    def _get_recent_activities(self, student_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        recent_evaluations = self.db.query(EvaluationAttempt).join(Evaluation).filter(
            EvaluationAttempt.user_id == student_id,
            EvaluationAttempt.tenant_id == tenant_id,
            EvaluationAttempt.status == 'completed'
        ).order_by(desc(EvaluationAttempt.completed_at)).limit(5).all()
        ...
        recent_milestones = self.db.query(LearningMilestone).join(LearningPath).filter(
            LearningPath.user_id == student_id,
            LearningPath.tenant_id == tenant_id,
            LearningMilestone.updated_at.isnot(None)
        ).order_by(desc(LearningMilestone.updated_at)).limit(5).all()
        ...
        for milestone in recent_milestones:
            sessions_data.append({
                ...
                'path_title': milestone.learning_path.title if hasattr(milestone, 'learning_path') else 'Öğrenme Yolu',
    ...
    def _get_enrollments(self, student_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        for enrollment in program_enrollments:
            programs_data.append({
                ...
                'title': enrollment.program.title if hasattr(enrollment, 'program') else f'Program {enrollment.program_id}',
                ...
                'completed_at': enrollment.completed_at.isoformat() if hasattr(enrollment, 'completed_at') else None
            })
        ...
        for enrollment in course_enrollments:
            courses_data.append({
                ...
                'title': enrollment.course.title if hasattr(enrollment, 'course') else f'Course {enrollment.course_id}',
                'status': enrollment.status,
                'progress': getattr(enrollment, 'progress', 0),
                'enrolled_at': enrollment.enrolled_at.isoformat() if hasattr(enrollment, 'enrolled_at') else None,
                'completed_at': enrollment.completed_at.isoformat() if hasattr(enrollment, 'completed_at') else None
    ...
    def _get_ai_analysis(self, student_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        recent_responses = self.db.query(QuestionResponse).join(EvaluationAttempt).filter(
            EvaluationAttempt.user_id == student_id,
            EvaluationAttempt.tenant_id == tenant_id,
    ...
    def _get_visualization_data(self, student_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        for week_offset in range(11, -1, -1):
            ...
            week_attempts = self.db.query(EvaluationAttempt).filter(
                EvaluationAttempt.user_id == student_id,
                EvaluationAttempt.tenant_id == tenant_id,
                EvaluationAttempt.status == 'completed',
    ...
    def _get_next_recommendations(self, student_id: int, tenant_id: int) -> Dict[str, Any]:
        ...
        if scores['risk_score'] == 'High':
            ...
            recommendations['action_items'] = [
                'Haftalık ilerleme takibi yapılmalı',
                'Motivasyon desteği sağlanmalı'
            ]
        else:
            recommendations['action_items'] = [
    ...
    def _get_last_activity_date(self, student_id: int, tenant_id: int) -> Optional[str]:
        ...
        last_attempt = self.db.query(EvaluationAttempt).filter(
            EvaluationAttempt.user_id == student_id,
            EvaluationAttempt.tenant_id == tenant_id
        ).order_by(desc(EvaluationAttempt.updated_at)).first()
    ...
    def _calculate_skill_distribution(self, student_id: int, tenant_id: int) -> List[Dict[str, Any]]:
        ...
        milestones = self.db.query(LearningMilestone).join(LearningPath).filter(
            LearningPath.user_id == student_id,
            LearningPath.tenant_id == tenant_id
        ).all()
    ...
    def _get_fallback_analysis(self, scores: Dict[str, Any], activities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if scores['risk_score'] == 'High':
            ...
            recommendations.extend([
                'Mevcut ilerleme takip edilmeli',
                'Güçlü yönler üzerine odaklanılmalı',
                'Yeni zorluklar sunulmalı'
            ])
