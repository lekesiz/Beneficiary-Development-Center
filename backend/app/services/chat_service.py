...
class ChatService(BaseService[Conversation]):
    ...
    def get_user_conversations(
        self, user_id: int, tenant_id: int, page: int = 1, per_page: int = 20, active_only: bool = True
    ) -> Dict[str, Any]:
    ...
    def get_or_create_conversation(self, user1_id: int, user2_id: int, tenant_id: int) -> Conversation:
        ...
        try:
            ...
            conversation = Conversation(user1_id=user1_id, user2_id=user2_id, tenant_id=tenant_id)
    ...
    def send_message(
        ...
        try:
            ...
            conversation = self.get_or_create_conversation(sender_id, receiver_id, tenant_id)
    ...
    def mark_messages_as_read(self, conversation_id: int, user_id: int, tenant_id: int) -> int:
    ...
    def delete_message(self, message_id: int, user_id: int, tenant_id: int) -> bool:
        """Soft delete a message for a user."""
        try:
            message = (
                self.db.query(ChatMessage)
                .filter(ChatMessage.id == message_id, ChatMessage.tenant_id == tenant_id)
                .first()
            )
