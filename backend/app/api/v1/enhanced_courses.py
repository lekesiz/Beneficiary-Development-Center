...
from app.schemas.enhanced_course import (
    CourseCreateSchema, CourseUpdateSchema, CourseQuerySchema,
    CourseResponseSchema, CourseSessionSchema, CourseReorderSchema,
    CourseDuplicateSchema, CourseStatisticsResponseSchema
)
                ...
                data['sessions'] = [
                    {
                        ...
                        'instructor_name': s.instructor.full_name if hasattr(s, 'instructor') and s.instructor else None,
...
def get_course(program_id, course_id):
    ...
    try:
        ...
        include_sessions = (
            request.args.get('include_sessions', 'false').lower() == 'true' or
            'sessions' in request.args.get('include', '').split(',')
        )
        include_progress = (
            request.args.get('include_progress', 'false').lower() == 'true' or
            'progress' in request.args.get('include', '').split(',')
        )
        include_materials = (
            request.args.get('include_materials', 'false').lower() == 'true' or
            'materials' in request.args.get('include', '').split(',')
        )
...
def delete_course(program_id, course_id):
    ...
    try:
        ...
        return ErrorResponse.create(
            ...
            message=str(e),
            status_code=400,
            error_code="CANNOT_DELETE_COURSE"
        )
    except Exception as e:
...
def duplicate_course(program_id, course_id):
    ...
    try:
        ...
        return ErrorResponse.create(
            ...
            message=str(e),
            status_code=409,
            error_code="DUPLICATE_COURSE_CODE"
        )
    except Exception as e:
