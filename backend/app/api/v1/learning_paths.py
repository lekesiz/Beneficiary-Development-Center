...
# Service instances will be created in request context
...
def suggest_learning_path_updates(student_id: int):
    ...
    return jsonify({"suggestions": [s.to_dict() for s in suggestions], "total": len(suggestions)})
...
def get_pending_updates(path_id: int):
    ...
    learning_path_service = LearningPathService()
    updates = learning_path_service.get_pending_updates(learning_path_id=path_id, tenant_id=user.tenant_id, user=user)

    return jsonify({"updates": [u.to_dict() for u in updates], "total": len(updates)})
...
def approve_update(update_id: int):
    ...
    # Get update
    update = (
        db.query(LearningPathUpdate)
        .filter(LearningPathUpdate.id == update_id, LearningPathUpdate.tenant_id == user.tenant_id)
        .first()
    )
    ...
    return jsonify({"message": "Update approved successfully", "update": update.to_dict()})
...
def reject_update(update_id: int):
    ...
    return jsonify({"message": "Update rejected successfully", "update": update.to_dict()})
...
def apply_update(update_id: int):
    ...
    learning_path = learning_path_service.apply_learning_path_update(
        update_id=update_id, tenant_id=user.tenant_id, user=user
    )

    return jsonify({"message": "Update applied successfully", "learning_path": learning_path.to_dict()})
...
def accept_learning_path(path_id: int):
    ...
    learning_path = learning_path_service.accept_learning_path(
        path_id=path_id, tenant_id=user.tenant_id, user=user, customization_notes=data.get("customization_notes")
    )
...
def update_learning_path(path_id: int):
    ...
    learning_path = learning_path_service.update_learning_path(
        path_id=path_id, tenant_id=user.tenant_id, user=user, updates=data
    )
...
def provide_feedback(path_id: int):
    ...
    learning_path = learning_path_service.provide_feedback(
        path_id=path_id, tenant_id=user.tenant_id, user=user, feedback=data["feedback"], rating=data.get("rating")
    )
...
def start_milestone(milestone_id: int):
    ...
    try:
        ...
        progress = (
            db.query(MilestoneProgress)
            .filter(MilestoneProgress.milestone_id == milestone_id, MilestoneProgress.user_id == user.id)
            .first()
        )
        ...
        return jsonify({"success": True, "message": "Milestone started successfully", "milestone_id": milestone_id})
...
@jwt_required()
def update_milestone_progress_standalone(milestone_id: int):
    """Update milestone progress."""
    user = get_jwt_identity()
    db = get_db()
    data = request.get_json()
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": "Progress updated successfully", "progress": progress.progress_percentage}
        )
...
def get_learning_path_statistics():
    ...
    learning_path_service = LearningPathService()
    paths = learning_path_service.get_user_learning_paths(user_id=user.id, tenant_id=user.tenant_id)
