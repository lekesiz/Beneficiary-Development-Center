...
from app.models.learning_path import LearningPath, LearningMilestone, MilestoneProgress
from app.models.evaluation import Evaluation, EvaluationAttempt
...
def get_student_dashboard():
    ...
    try:
        ...
        recent_achievements = []
        recent_milestones = db.query(MilestoneProgress).filter(
            MilestoneProgress.user_id == user.id,
            MilestoneProgress.status == 'completed',
            MilestoneProgress.completed_at != None
        ).order_by(MilestoneProgress.completed_at.desc()).limit(5).all()
        ...
        in_progress_milestones = db.query(MilestoneProgress).filter(
            MilestoneProgress.user_id == user.id,
            MilestoneProgress.status == 'in_progress'
        ).all()
        ...
        for progress in in_progress_milestones:
            if progress.started_at:
                ...
                if days_since_start > 7:
                    alerts.append({
                        'type': 'warning',
                        'message': f"{progress.milestone.title} için 7 günden fazla süre geçti"
                    })
        ...
        if overall_progress < 25 and total_milestones > 0:
            alerts.append({
                'type': 'info',
                'message': 'Öğrenme hedeflerinizde ilerleme kaydetmeyi unutmayın!'
            })
